...
def generate(input, subData, closed_loop_dict, pid_dict, arm_disarm_value, arm_inputs):
	...
	if (s2_mapped > 30 or s2_mapped < 380): s2_mapped +=(servo2 * mult)
	if (s2_mapped < 10):
		s2_mapped = 10
	elif (s2_mapped > 420):
		s2_mapped = 420
	...
	if (s3_mapped > 30 or s3_mapped < 380): s3_mapped += ((servo3 + 0.5) * 2 * mult)
	if (s3_mapped < 10):
		s3_mapped = 10
	elif (s3_mapped > 420):
		s3_mapped = 420
	...
	if (s4_mapped > 110 or s4_mapped < 200): s4_mapped += (servo4 * mult)
	if (s4_mapped < 100):
		s4_mapped = 100
	elif (s4_mapped > 200):
		s4_mapped = 200
